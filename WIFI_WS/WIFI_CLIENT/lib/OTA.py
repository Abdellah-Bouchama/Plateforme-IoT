...
class OTA():
    ...
    def get_update_manifest(self):
        #device_id = '123'  # Remplacez par l'ID de votre appareil
        #Obtenez l'adresse MAC 
        mac = ubinascii.hexlify(machine.unique_id()).decode()
        device_id =nodeId
        print("Adresse MAC de la carte FiPy:", mac)
        #req = "manifest.json?current_ver={}".format(self.get_current_version())
        req = "manifest.json?current_ver={}&device_id={}".format(self.get_current_version(), device_id)
